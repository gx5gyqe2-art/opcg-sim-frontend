...
export const SandboxGame = ({ p1Deck, p2Deck, onBack }: { p1Deck: string, p2Deck: string, onBack: () => void }) => {
  ...
  useEffect(() => {
    ...
    if (inspecting) {
        const overlay = createInspectOverlay(
            inspecting.type,
            inspectingCards,
            revealedCardIds, // ★追加: 現在表向きのカードID
            W, H,
            () => setInspecting(null),
  ...
  }, [gameState, isPending, dragState, inspecting, isRotated, inspectingCards, revealedCardIds]); // revealedCardIdsを追加
        ...
        if (distFromStart < 10) {
            ...
            if (currentPlayer) {
                ...
                if (stackInfo) {
                    ...
                    setRevealedCardIds(new Set()); // ★追加: 開くたびに表向き状態リセット
                    setDragState(null);
                    return;
                }
            }
