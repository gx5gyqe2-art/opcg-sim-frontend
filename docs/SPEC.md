...
- 効果処理：対象選択・任意確認・トリガー解決のオーバーレイ／`ui/CardSelectModal.tsx`。サーバが候補実体（`candidates`）を送らず `selectable_uuids` のみの場合、`modalCandidates` は**両プレイヤーの全ゾーン**（手札/場/ライフ/トラッシュ/デッキ/ドン!!/リーダー/ステージ）から実体を引く（手札・場・リーダーのみだと蘇生＝トラッシュ・デッキ操作・ライフ操作・ドン返却等で候補が空になり選べず停止し得たため）。
